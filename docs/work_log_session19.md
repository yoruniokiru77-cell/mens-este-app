...
### 売上編集モーダルのキャッシュ依存（次セッションで改善予定）

**現状の制約：** 売上編集モーダルの給料・店落ちプレビューは `window._payrollMenuBackMap` 等のグローバルキャッシュを参照しているが、このキャッシュは給料計算ページを開いたとき（`getPayroll`実行時）にのみセットされる。

→ 給料計算ページを開かずに売上編集モーダルを開くと、固定バックが参照できずバック率計算にフォールバックする。

**改善案（次セッションで実装）：** `openSalesEditModal` 実行時にキャッシュが未設定であれば自動でデータ取得する処理を追加する。
