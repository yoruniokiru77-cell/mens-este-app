...
export function _normalizeTime(timeStr: string): string {
  if (!timeStr) return '';
...
export function _timeToMinutes(timeStr: string): number {
  if (!timeStr) return 0;
  const [h, m] = timeStr.split(':').map(Number);
  return h * 60 + (m || 0);
}

export function _timeLabel(timeStr: string, _dateStr?: string): string {
  if (!timeStr) return '';
  return timeStr.substring(0, 5);
}
...
export function _fmtDatetimeJp(isoStr: string): string {
  if (!isoStr) return '';
...
export function _fmtLocalDatetimeJp(localStr: string): string {
  if (!localStr) return '';
...
export function _fmtDateJp(isoStr: string): string {
  if (!isoStr) return '';
  return isoStr.substring(0, 10).replace(/-/g, '/');
}

export function _fmtTimeJp(timeStr: string): string {
  return (timeStr || '').substring(0, 5);
}

// 時間文字列を分数に変換（27時ルール：00〜02時台は24+hとして扱う）
export function _timeToMin27(t: string): number {
  if (!t) return 0;
